...
# INPUTS: 
#   - two arguments (type not specified)
# OUTPUTS: 
#   - returns `true` or `false`
# RULES:
#   - returns `true` if exactly one of the arguments is truthy
#   - returns `false` if both arguments are truthy or both arguments are falsy

# ALGORITHM:
#   Create method called `xor(first, second)`
